...
exports.finalUpdateValidation = [
  async (req, res, next) => {
            ...
            await body('interestedIn').notEmpty().withMessage(validaionError.GENDER_VALIDATION_ERROR).run(req)
            ...
            await body('images').notEmpty().trim().isLength({min: 1}).isLength({max:2000}).withMessage(validaionError.ABOUT_ME).run(req)
